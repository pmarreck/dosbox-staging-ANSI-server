...
class TextModeServerApiTest : public ::testing::Test {
protected:
	void SetUp() override
	{
		save_vga_state();
		textmode::Configure(textmode::ServiceConfig{});
	}
	...
	static void PrepareTextModeFrame()
	{
		...
		vga.draw.cursor.address     = 0;
		vga.draw.blinking           = 0;
		vga.draw.blink              = false;
	}

	static std::string ExpectedFrame()
	{
		return "*META cols=2\n"
		       "*META rows=1\n"
...
TEST_F(TextModeServerApiTest, ReportsDisabledWhenConfiguredOff)
{
	const auto response = textmode::HandleCommand("GET");

	EXPECT_FALSE(response.ok);
	EXPECT_EQ(response.payload, "ERR text-mode server disabled\n");
}

TEST_F(TextModeServerApiTest, ReturnsFrameWhenEnabled)
{
	PrepareTextModeFrame();
	...
	EXPECT_EQ(response.payload, ExpectedFrame());
}

TEST_F(TextModeServerApiTest, ShimReturnsPayloadString)
{
	PrepareTextModeFrame();
	...
	textmode::Configure(config);
	const auto payload = TEXTMODESERVER_HandleCommand("GET");

	EXPECT_EQ(payload, ExpectedFrame());
}

TEST_F(TextModeServerApiTest, ConfigInitBindsService)
{
	PrepareTextModeFrame();

	CommandLine command_line(0, nullptr);
	auto config = std::make_unique<Config>(&command_line);

	TEXTMODESERVER_AddConfigSection(config);
	auto* section = dynamic_cast<SectionProp*>(config->GetSection("textmode_server"));
	ASSERT_NE(section, nullptr);

	ASSERT_TRUE(section->HandleInputline("enable = true"));
	ASSERT_TRUE(section->HandleInputline("show_attributes = false"));
	ASSERT_TRUE(section->HandleInputline("sentinel = *"));

	const auto before_init = textmode::HandleCommand("GET");
	EXPECT_FALSE(before_init.ok);
